...
**Angular 21 monorepo** with **Turborepo + pnpm workspaces**. Two
applications (`apps/web` for end users, `apps/admin` for administration)
share seven internal packages under the `@tkf/*` npm scope.

```
┌─────────────────────────────────────────────────────────────┐
│                       apps/                                 │
│   ┌────────────────────┐       ┌────────────────────┐       │
│   │  web  (port 4200)  │       │ admin (port 4300)  │       │
│   └─────────┬──────────┘       └─────────┬──────────┘       │
│             │                              │                 │
└─────────────┼──────────────────────────────┼─────────────────┘
              │                              │
...
### AD-02 · `web` and `admin` are separate apps
Blast-radius isolation (admin crash ≠ user-facing outage), independent
bundles, distinct RBAC. Both consume the same `@tkf/*` packages.
...
component. *(Initial wiring ships in Phase 1; this phase only ships the
component code.)*
...
### AD-12 · `@angular/localize` for i18n (pt-BR + en-US)
Angular 21's native i18n is sufficient for two locales; no need for
third-party libraries.

### AD-13 · JWT + Refresh + RBAC scaffolded from day zero
`core/auth` ships with: `JwtService`, `AuthHttpInterceptor`, `AuthStore`,
`AuthGuard`, `HasRoleDirective`. Roles: `admin | manager | member | viewer`.
Logic is wired but the login UI is deferred to Phase 1.

### AD-14 · Multi-stage Docker → nginx-alpine
Each app ships its own `Dockerfile`. Final image is nginx-alpine with
hashed static assets and SPA fallback. Sub-50 MB target.
...
### AD-17 · No features in Phase 0
Only `health-check` (web) and `admin-dashboard` placeholder (admin)
exist. Everything else is scaffolding.
